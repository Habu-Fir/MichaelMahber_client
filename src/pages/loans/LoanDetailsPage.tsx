...
const LoanDetailsPage = () => {
    ...
    // Better owner check with proper ID comparison
    const userId = user?._id?.toString();
    const loanMemberId = loan.memberId?._id?.toString();
    const isOwner = !!(userId && loanMemberId && userId === loanMemberId);
    ...
    const hasUserSigned = loan.signatures?.some(s => {
        const sigMemberId = s.memberId?._id?.toString() || s.memberId?.toString();
        return sigMemberId === userId;
    });
    ...
    // 2. Loan status is 'active' (case insensitive)
    ...
    console.log('💰 Payment Permission:', {
        isOwner,
        status: loan.status,
        statusLower: loan.status?.toLowerCase(),
        remainingPrincipal: loan.remainingPrincipal,
        canMakePayment,
        hasPendingPayments
    });
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 pb-8">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className="p-3 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors flex items-center gap-2"
            ...
            {/* Debug Info Panel */}
            {showDebugInfo && (
                <div className="bg-gray-900 text-white rounded-2xl p-6 font-mono text-sm overflow-auto">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <p className="text-gray-400 mb-1">Loan Owner Check:</p>
                            <pre className="bg-gray-800 p-3 rounded-lg">
                                {JSON.stringify({
                                    userId,
                                    loanMemberId,
                                    isOwner,
                                    match: userId === loanMemberId
                                }, null, 2)}
                            </pre>
                        </div>

                        <div>
                            <p className="text-gray-400 mb-1">Loan Status:</p>
                            <pre className="bg-gray-800 p-3 rounded-lg">
                                {JSON.stringify({
                                    status: loan.status,
                                    statusLower: loan.status?.toLowerCase(),
                                    remainingPrincipal: loan.remainingPrincipal,
                                    remainingPrincipalValid: (loan.remainingPrincipal || 0) > 0
                                }, null, 2)}
                            </pre>
                        </div>

                        <div>
                            <p className="text-gray-400 mb-1">Payment Permission Calculation:</p>
                            <pre className="bg-gray-800 p-3 rounded-lg">
                                {JSON.stringify({
                                    isOwner,
                                    statusCondition: loan.status?.toLowerCase() === 'active',
                                    principalCondition: (loan.remainingPrincipal || 0) > 0,
                                    canMakePayment
                                }, null, 2)}
                            </pre>
                        </div>
                        ...
                        <div>
                            <p className="text-gray-400 mb-1">All Conditions:</p>
                            <pre className="bg-gray-800 p-3 rounded-lg">
                                {JSON.stringify({
                                    canSign,
                                    canApprove,
                        ...
                        <div>
                            <p className="text-gray-400 mb-1">Raw Loan Data:</p>
                            <pre className="bg-gray-800 p-3 rounded-lg max-h-60 overflow-auto">
            ...
            <div className="flex flex-wrap gap-4 mt-6 mb-8">
                ...
                {/* TEMPORARY TEST BUTTON - Shows for all active loans regardless of ownership */}
                {loan.status?.toLowerCase() === 'active' && (
                    <button
                        onClick={() => navigate(`/loans/${id}/pay`)}
                        className="px-8 py-4 bg-purple-600 hover:bg-purple-700 text-white font-bold text-lg rounded-xl transition-all transform hover:scale-105 active:scale-95 flex items-center gap-3 shadow-xl shadow-purple-600/40 border border-purple-400"
                    >
                        <Bug className="w-6 h-6" />
                        TEST PAYMENT (Active Loan)
                    </button>
                )}
