...
const LoanDetailsPage = () => {
    ...
    // ✅ NEW: Update interest in database when page loads
    useEffect(() => {
        const updateInterestInDatabase = async () => {
            if (!id) return;

            setIsUpdatingInterest(true);
            try {
                console.log('💰 Updating interest for loan:', id);
                await api.post(`/loans/${id}/update-interest`);
                // Refetch loan data to get updated values
                await refetch();
